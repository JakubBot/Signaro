...
MAX_INFERENCE_WORKERS = 2  # przykładowa wartość
...
class VideoTransformTrack(MediaStreamTrack):
    ...
    def __init__(self, track):
        ...
        # MediaPipe Holistic model (shared for this track)
    ...
    async def recv(self):
        ...
        if should_schedule:
            task = asyncio.create_task(
                self._schedule_prediction(img.copy())
            )
    ...
    async def _schedule_prediction(self, img):
        async with inference_semaphore:
            ...
            result = await loop.run_in_executor(self._executor, self._sync_predict, img)
            if result is not None:
                self._latest_result = result

    def _sync_predict(self, img):
        try:
            output_img, results = mediapipe_detection(img, self._holistic)
